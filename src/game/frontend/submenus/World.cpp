...
namespace YimMenu::Submenus
{
	    ...
	    Submenu::Submenu("世界", ICON_FA_GLOBE)
	{
		...
		auto iplsGroup = std::make_shared<Category>("IPL");
		...
		auto bringOpts = std::make_shared<Group>("召唤", 1);
		...
		auto timeGroup = std::make_shared<Group>("时间");

		auto hms = std::make_shared<Group>("", 1);
hms->AddItem(std::make_shared<IntCommandItem>("networktimehour"_J, "时"));
		hms->AddItem(std::make_shared<IntCommandItem>("networktimeminute"_J, "分"));
