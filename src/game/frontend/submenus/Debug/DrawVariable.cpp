...
namespace YimMenu
{
	...
	void DrawSavedVariablePreview(const SavedVariableBase& var, void* value)
	{
		...
		case VariableType::BOOL:
		{
			ImGui::Selectable(*static_cast<bool*>(value) ? "是" : "否", false, ImGuiSelectableFlags_Disabled);
			break;
		}
		case VariableType::FLOAT:
		{
	...
	void DrawSavedVariable(SavedVariableBase& var)
	{
		...
		for (int i = 0; i < var.appendages.size(); i++)
		{
			...
			case VariableAppendage::Type::OFFSET:
				...
				ImGui::InputScalar(std::format("偏移##{}{}", i, (int)item.type).c_str(), ImGuiDataType_S32, &item.offset, &step, &step_fast);
				ImGui::SameLine();
				ImGui::SetNextItemWidth(125.f);
